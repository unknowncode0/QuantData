...
class QDFactor():
    def __init__(self, factor_name,host, port, user, password):
        self.client = clickhouse_driver.Client(host=host, port=port, user=user, password=password,
                                               database='factor')

        self.client.execute('CREATE TABLE IF NOT EXISTS  `factor`.`factormetadata`(\
                factorname String, \
                create_date Date, \
                update_date Date, \
                version Float32, \
    ...
    def __str__(self):
        return "QDFACTOR {}".format(self.factor_name)
    ...
    def update_to_database(self,data):
